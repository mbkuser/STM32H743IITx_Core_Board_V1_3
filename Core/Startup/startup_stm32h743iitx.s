...
/*-----------------------------------------------*/
/* Copy .itcm_text from Flash to ITCM RAM: копирование из Flash в ITCM при старте */
/*Альтернатива без правки стартапа: используй функцию __attribute__((constructor)) в C*/
/*или сделать копирование в самом начале main() через memcpy (альтернативный способ).*/
  ldr r0, =_sitcm_text	        /* destination start (ITCM) */
  ldr r1, =_eitcm_text	        /* destination end   (ITCM) */
  ldr r2, =_sitcm_text_load		/* source start      (Flash) */
  movs r3, #0
  b LoopCopyITCM
...
LoopCopyITCM:
  ...
  /* ── Копируем .dtcm_data из Flash в DTCM ── */
  ldr r0, =_sdtcm_data
  ldr r1, =_edtcm_data
  ldr r2, =_sdtcm_data_load
  b   LoopCopyDTCM
CopyDTCM:
  ldr r3, [r2], #4
  str r3, [r0], #4
...
/* Call static constructors */
    bl __libc_init_array	/* ← здесь вызываются constructors */
/* Call the application's entry point.*/
  bl  main					/* ← только потом main() */
  bx  lr
